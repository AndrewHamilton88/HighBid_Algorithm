            ...
            using (StreamWriter WriteFile = new StreamWriter((NetPath + "\\" + FileName))) // create a new text file to write to.
            {
                try
                {
                    ...
                    foreach (N_W_Link Link in LinkList)
                    {
                        ...
                        WriteFile.WriteLine("link " + Link.StartNode + " " + Link.EndNode + " category " + Link.catagory.ToString() + " force merge force across");
                        if (Link.OneWay == false)
                        {
                            WriteFile.WriteLine("link " + Link.EndNode + " " + Link.StartNode + " category " + Link.catagory.ToString() + " force merge force across toll 0.000");
                        }
                    }
                }
                catch (Exception e)
